...
def truthTableSim(logic, POS_Format, SOP_Format):
    ...
    for i in logic:
        if i.isalpha():
            if i not in inputs: inputs.append(i)
    inputs = sorted(inputs)
    # Individual terms from the statement
    terms = []
    ...
    counter = 1
    # Dictionary for storing bool for base values
    # Dictionary for storing the bool value of the final term
    # Dictionary for storing bool for specific terms separated by spaces
    boolInputs,termBools, logicBools = {}, {}, {}

    # Append the complementary inputs in the inputs as well for the final bool values
    for i in range(len(inputs)): inputs.append(inputs[i]+"'")
    ...
    logicBools[logic] = mainLogicTable(terms, termBools, realNumOfInputs, POS_Format, SOP_Format)

    # Write in file all the terms and their truth table separately
    file.write("\nLogical Terms\n")
    for term in terms: file.write(f"{term} : {termBools[term]}\n")
...
# Update the value of counter accordingly
    ...
    if SOP_Format:
        for i in range(len(term)):
            ...
            if term[i:i+2] == term[i] + "'" or term[i:i+2] == term[i] + "`":
                ...
                i += 1
            elif term[i].isalpha(): termInputs.append(term[i])
...
def logicalEquivalence(statementA, statementB):
    boolValuesA = truthTableSim(statementA, getFormat(statementA)[0], getFormat(statementA)[1])
    boolValuesB = truthTableSim(statementB, getFormat(statementB)[0], getFormat(statementB)[1])
...
# The two formats are S.O.P(Sum of Products [Minterms]) and P.O.S(Products of Sum [Maxterms])


def getFormat(statement):
    POS_Format = False
    SOP_Format = False
    if "(" in statement: POS_Format = True
    else: SOP_Format = True
    return [POS_Format, SOP_Format]


if __name__ == "__main__":
    # Create a bool value for if the equation is valid
    validEqn, symbols = False, "'+`() "
    ...
    logicalEqv, validAnswer = False, False
    # Empty string for storing user inputs
    chooseFunc, logic, statementA, statementB = "", "", "", ""
    while not validAnswer:
        chooseFunc = input(
            "Type one of the options\n 1 for Logical Equivalence Calculator \n2 for Truth Table Simulator: \n")
    ...
    if logicalEqv:
        ...
        while not validEqn:
            ...
            # For every character in the statement, if the statement contains that are not valid, make validEqn false
            for i in statementA:
